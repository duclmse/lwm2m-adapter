...
public class LeshanServerDemo {
    ...
    public static LeshanServer createLeshanServer(LeshanServerDemoCLI cli) throws Exception {
        ...
        if (cli.identity.isx509()) {
            ...
            if (dtlsConfig.getIncompleteConfig().get(DtlsConfig.DTLS_ROLE) == DtlsRole.BOTH && serverCertificate != null
                && CertPathUtil.canBeUsedForAuthentication(serverCertificate, false)
                && !CertPathUtil.canBeUsedForAuthentication(serverCertificate, true)) {
    ...
    private static Server createJettyServer(LeshanServerDemoCLI cli, LeshanServer lwServer) {
        ...
        if (cli.identity.isRPK()) {
            securityServletHolder = new ServletHolder(
                new SecurityServlet((EditableSecurityStore) lwServer.getSecurityStore(), cli.identity.getPublicKey()));
        } else {
            securityServletHolder = new ServletHolder(
                new SecurityServlet((EditableSecurityStore) lwServer.getSecurityStore(),
                    cli.identity.getCertChain()[0]));
        }
